...
export default function Home() {
  ...
  const handleDashboardClick = () => navigate(token ? "/dashboard" : "/login");
  const handleLogin = () => navigate("/login");
  const handleSignup = () => navigate("/register");

  return (
    <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 min-h-screen text-white flex flex-col scroll-smooth">
      {/* 🌐 Header */}
      <header className="fixed top-0 w-full z-50 bg-white/10 backdrop-blur-lg shadow-lg">
        <nav className="flex justify-between items-center px-6 py-4">
          <h1 className="text-2xl text-gray-800 font-bold">Mini-CRM</h1>

          {/* Desktop Nav */}
          <div className="hidden md:flex space-x-6">
            <button onClick={handleDashboardClick} className="text-black hover:text-gray-200">Dashboard</button>
            <button onClick={handleLogin} className="text-black hover:text-gray-200">Login</button>
            <button onClick={handleSignup} className="text-black hover:text-gray-200">Signup</button>
            <Link
              to="/register"
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="md:hidden bg-white text-black flex flex-col items-center py-6 space-y-4 shadow-md z-40">
            <button onClick={handleDashboardClick} className="w-4/5 py-2 rounded hover:bg-gray-100">Dashboard</button>
            <button onClick={handleLogin} className="w-4/5 py-2 rounded hover:bg-gray-100">Login</button>
            <button onClick={handleSignup} className="w-4/5 py-2 rounded hover:bg-gray-100">Signup</button>
            <Link
              to="/register"
              className="w-4/5 px-4 py-2 bg-yellow-400 rounded-xl font-semibold text-center hover:bg-yellow-300 transition"
            >
              Get Started
            </Link>
          </div>
      ...
      {/* 🎯 Hero Section */}
      <main className="flex flex-1 items-center justify-center text-center px-4 sm:px-6 pt-24 md:pt-32">
      ...
      <section className="py-20 bg-white text-gray-900 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-4 sm:px-10">
          <motion.div whileHover={{ scale: 1.05 }} className="p-6 bg-gray-50 rounded-xl shadow hover:shadow-lg transition min-h-[200px]">
